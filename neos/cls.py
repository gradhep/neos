...
__all__ = ['cls_maker', 'cls_maker2', 'pyhf_cls_maker']
...
def cls_maker2(nn_model_maker1, nn_model_maker2, solver_kwargs):
    @jax.jit
    def cls_jax(nn_params, test_mu):
        g_fitter, c_fitter = get_solvers(nn_model_maker1, **solver_kwargs)

        m, bonlypars = nn_model_maker1(nn_params)
        exp_data = m.expected_data(bonlypars)
        bounds = m.config.suggested_bounds()
        ...
        # MODEL 2
        g_fitter, c_fitter = get_solvers(nn_model_maker2, **solver_kwargs)

        m, bonlypars = nn_model_maker2(nn_params)
        exp_data = m.expected_data(bonlypars)
        #print(f'exp_data: {exp_data}')
        bounds = m.config.suggested_bounds()
