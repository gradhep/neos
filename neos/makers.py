...
def hists_from_nn(
    data_generator, predict, method="softmax", LUMI=10, sig_scale=2, bkg_scale=10
):
    ...
    if len(data) == 3:
        if method == "softmax":
            ...
            assert (
                False
            ), f"Unknown soft histogram method '{method}'. Currently only 'softmax' or 'kde' are available."
